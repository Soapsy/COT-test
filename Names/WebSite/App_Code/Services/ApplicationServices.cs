...
using TestCompany.Services.Rest;
...
namespace TestCompany.Services
{
        ...
        public static string ValidationKey
        {
            get
            {
                ...
                if (string.IsNullOrEmpty(key) || key.Contains("AutoGenerate"))
                    key = "1F9B8ED8324BCCEAF01796BDF9C9FEC698677AC2576AFB295FE9BB93D612CDB47ED8DFD7AA1BBD26451F8B7A0683F5BB92BCCAA5501C699AEB3E1E448D64878D";
                return key;
            }
        }
    ...
    public partial class ApplicationServicesBase
    {
        ...
        public virtual void EnumerateOfflineUserResources(List<string> pages, List<string> controllers)
        {
            var map = EnumerateAppPages();
            foreach (var page in map.Properties())
                if (Convert.ToBoolean(page.Value["offline"]))
                {
                    var pageRoles = ((JArray)(page.Value["roles"]));
                    if (pageRoles != null)
                        foreach (string role in pageRoles)
                            if ((role == "*") || DataControllerBase.UserIsInRole(role))
                            {
                                pages.Add(page.Name);
                                var pageControllers = page.Value["controllers"];
                                if (pageControllers != null)
                                    foreach (string controller in pageControllers)
        ...
        public static void RegisterCssLinks(Page p)
        {
            ...
            if (ApplicationServices.EnableCombinedCss)
            {
                if (ApplicationServicesBase.IsUsingResourceBundling)
                    l.Href = p.ResolveUrl(AppResourceManager.ToResourceName("~/app.all.min.css"));
                else
        ...
        public virtual JObject JsonError(Exception ex, JObject exInfo, string error, string description, params object[] args)
        {
            ...
            if (ex is RESTfulResourceException)
                ...
                {
                    var relatedDetails = new JObject(new JProperty("reason", related.Error), new JProperty("description", related.Message));
        ...
        public virtual bool HandleWebAppRequest()
        {
            ...
            var combined = Regex.Match(filePath, "(/|\\\\)app.all(\\.(?'Culture'\\w+\\-\\w+))?\\.min\\.(?'Type'css|js)$");
            if (combined.Success)
            {
