...
namespace TestCompany.Services
{
    ...
    public partial class ApplicationServicesBase
    {
        ...
        public virtual UserTicket CreateTicket(MembershipUser user, string refreshToken, int accessTokenDuration, int refreshTokenDuration)
        {
            ...
            var canWorkOffline = UserCanWorkOffline(user.UserName);
            if (string.IsNullOrEmpty(refreshToken))
            {
                if (canWorkOffline)
                    refreshTokenDuration = GetRefreshTokenDuration("odp.sync.refreshTokenDuration", 180);
            ...
            if (canWorkOffline)
                ticket.Claims["urn:app:remember-me"] = true;
            return ticket;
        }
        ...
        public virtual object AuthenticateUser(string username, string password, System.Boolean? createPersistentCookie)
        {
            ...
            else
            {
                ...
                {
                    if (UserCanWorkOffline(username))
        ...
        public virtual bool UserCanWorkOffline(string username)
        {
            try
            {
                return Roles.GetRolesForUser(username).Contains("Offline");
            }
            catch (Exception)
            {
                return false;
        ...
        public virtual void EnumerateIdClaims(MembershipUser user, JObject claims, List<string> scopes)
        {
        }
    }
}
