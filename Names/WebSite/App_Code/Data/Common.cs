...
namespace TestCompany.Data
{
    ...
    public class AppResourceManager
    {
        ...
        public static string ToHash(string resourceName)
        {
            ...
            if (Regex.IsMatch(resourceName, "app.all.min.css"))
                cacheKey = string.Format("{0};{1};{2}", cacheKey, ApplicationServicesBase.Current.UserTheme, ApplicationServicesBase.Current.UserAccent);
            ...
            if (string.IsNullOrEmpty(fileHash))
            {
                ...
                else
                {
                    ...
                    var combinedResource = Regex.Match(resourceName, "(/|\\\\)app\\.all(\\.\\w+\\-\\w+)?\\.min\\.(?'Type'css|js)$");
